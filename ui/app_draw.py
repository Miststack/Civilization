...
class DrawMixin:
    ...
    def _draw_sidebar_legend(self, box: pygame.Rect) -> None:
        ...
        sections: list[tuple[str, str, list[tuple[str, str, str]]]] = [
            (
                "地形",
                "terrain",
                [(tt.value, label, terrain_yield_hint(tt)) for tt, label in TERRAIN_LABELS.items()],
            ),
            (
                "建筑",
                "building",
                [(b.value, label_building(b), building_terrain_hint(b)) for b in BuildingType],
            ),
            (
                "标记",
                "marker",
    ...
    def _draw_tech_row(self) -> None:
        ...
        unlocked = {label_tech(t) for t in self.state.tech_unlocked}
    ...
    def _draw_panel(self) -> None:
        ...
        if self.human_mode and not self.game_over:
            ...
            if self._legend_open:
                ...
                hist_box = pygame.Rect(
                    action_card.x + 8,
                    action_card.y + self.layout.px(28),
                    action_card.width - 16,
                    action_card.bottom - action_card.y - self.layout.px(36),
                )
                self._draw_action_history(hist_box)
                ...
                draw_text(
                    self.screen,
                    self.font_tiny,
                    f"点击高亮格建城  ·  {short_resource_cost(costs)}  ·  {CITY_BUILD_TURNS}回合",
            ...
            if self._legend_open:
                ...
                hint = "空格暂停  N单步  [ 减速  ] 加速  ·  L 图例"
                if self._paused:
                    hint = "已暂停  ·  N 执行一步  ·  空格继续  ·  L 图例"
                draw_text(
                    self.screen,
                    self.font_tiny,
                    hint,
    ...
    def _draw_footer(self) -> None:
        ...
        if self.human_mode and not self.game_over and self.hover_cell is None and y + 16 < self.footer_rect.bottom:
            draw_text(
                self.screen,
                self.font_tiny,
                "1~4 动作 · F5/F9 存读 · T 主题 · Z 撤销 · H 历史 · L 图例",
    ...
    def _draw_action_history(self, box: pygame.Rect) -> None:
        ...
        if not self._action_history:
            draw_text(self.screen, self.font_small, "（尚无记录）", (box.x + 12, y), self.theme.text_dim)
            return
        for turn, label in reversed(self._action_history[-14:]):
            if y + 18 > box.bottom - 8:
                break
            draw_text(
                self.screen,
                self.font_tiny,
    ...
    def _draw_thinking_overlay(self) -> None:
        overlay = pygame.Surface(self.map_rect.size, pygame.SRCALPHA)
        overlay.fill((*self.theme.overlay, 160))
        self.screen.blit(overlay, self.map_rect.topleft)
        elapsed = pygame.time.get_ticks() - self._think_started_at
        dots = "." * (1 + (elapsed // 400) % 3)
        label = f"计划搜索思考中{dots}"
        text = self.font_title.render(label, True, self.theme.text)
        box = text.get_rect(center=self.map_rect.center)
        box.inflate_ip(48, 24)
        surf = pygame.Surface(box.size, pygame.SRCALPHA)
        pygame.draw.rect(surf, (*self.theme.header_bg, 230), surf.get_rect(), border_radius=12)
        pygame.draw.rect(surf, (*self.theme.accent, 200), surf.get_rect(), width=2, border_radius=12)
    ...
    def _draw_city_detail(self, action_card: pygame.Rect) -> None:
        ...
        if city.buildings:
            bnames = "、".join(label_building(b) for b in sorted(city.buildings, key=lambda x: x.value))
        else:
            bnames = "无建筑"
        ...
        draw_text(
            self.screen,
            self.font_tiny,
            f"3×3产出: {format_yields_static(yields)}  ·  维护 -{CITY_MAINTENANCE_FOOD}粮",
            (box.x + 10, box.y + 46),
            self.theme.text_dim,
        )
