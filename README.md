...
- `--play`：`human`（手动） / `random`（随机） / `greedy`（规则贪心） / `planned`（计划束搜索）；省略则启动时询问（1～4）。  
- `--quiet`：自动模式下减少打印。  
...
- **选种子**：`--seed` 后跟整数。  
- **选运行方式**：`--play human` / `random` / `greedy` / `planned`。  
...
当前共 **52** 项用例，覆盖：游戏规则、`main` 冒烟（含 `planned`）、搜索模块与计划智能体等。**测试文件由 AI 辅助编写**，本人负责核对断言与实现是否一致；详见 `课程设计报告.md` §3.2。
